...
def run_episode(env, policy_net, value_net, maxlen=1000):
...
def reinforce(policy_net, value_net, env, env_render=None, gamma=0.99, num_episodes=10, eval_interval=100, eval_episodes=10,
    ...
    for episode in range(num_episodes):
        ...
        if episode % eval_interval == 0:
            ...
            print(
                f'Episode {episode} Average Reward: {total_reward / eval_episodes}, Average Episode Length: {episode_length / eval_episodes}')
            if display:
...
if __name__ == "__main__":
    ...
    plt.xlabel('# Episodes')
    plt.ylabel('Reward')
    # plt.savefig('./images/LunarLander_basicRL/Running_rewards.png')  # Save the figure
    ...
    plt.xlabel('# Episodes')
    plt.ylabel('Reward')
    # plt.savefig('./images/LunarLander_basicRL/Average_rewards.png')  # Save the figure
    ...
    plt.xlabel('# Episodes')
    plt.ylabel('Length')
    # plt.savefig('./images/LunarLander_basicRL/Average_episode_length.png')  # Save the figure
    ...
    env_render.close()
    if standardize:
        save_frames_as_gif(frames, path='./images/LunarLander_basicRL/', filename='LunarLander_basic.gif')
    else:
        save_frames_as_gif(frames, path='./images/LunarLander_basicRL/', filename='LunarLander_basic_nostd.gif')
